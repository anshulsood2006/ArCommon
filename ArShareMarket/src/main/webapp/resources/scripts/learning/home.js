...
module.controller('learningHomeController', function($scope, $http) {
	$scope.getRequest = function(){
		$http.get("http://arjboss:8080/arcommon/services/dukaan/product/get", {headers:{'Content-Type': 'application/json','Authorization':'Basic YW5zaHVsOnBhc3N3b3Jk'}})
		.then(
	...
	$scope.postRequest = function(){
		$http.post("http://arjboss:8080/arcommon/services/dukaan/product/add", data, {headers:{'Content-Type': 'application/json','Authorization':'Basic YW5zaHVsOnBhc3N3b3Jk'}})
		.then(
				function successCallback(response){
					$scope.response = response;
					console.log(response.data);
